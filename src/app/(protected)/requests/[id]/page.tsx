...
function mapDetail(json: any, id: string): Detail {
  ...
  const codes = json.codes ?? json.procedures ?? (json.code ?? json.procedure_code)
  const diagnoses = json.diagnosisCodes ?? json.diagnoses ?? json.icd10_codes
  ...
  return {
    ...
    memberName:
      json.memberName ?? json.member?.name ?? json.patientName ?? json.patient?.name ?? undefined,
    memberId:
      json.memberId ?? json.member?.id ?? json.patientId ?? json.patient_id ?? json.patient?.id ?? undefined,
    memberDob:
       json.memberDob ?? json.member?.dob ?? json.patientDob ?? json.patient?.dob ??
       json.member?.date_of_birth ?? json.patient?.date_of_birth ?? undefined,
    providerName: json.providerName ?? json.provider?.name ?? undefined,
    providerNpi: json.providerNpi ?? json.provider?.npi ?? json.provider_npi ?? json.npi ?? undefined,
    codes: toStrArray(codes),
    diagnosisCodes: toStrArray(diagnoses),
